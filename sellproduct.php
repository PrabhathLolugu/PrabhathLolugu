...
if(isset($_REQUEST['psubmit'])){
  ...
  if(($_REQUEST['cname'] == "") || ($_REQUEST['cadd'] == "") || ($_REQUEST['pname'] == "") || ($_REQUEST['pquantity'] == "") || ($_REQUEST['psellingcost'] == "") || ($_REQUEST['totalcost'] == "") || ($_REQUEST['selldate'] == "")){
    ...
    // Assigning User Values to Variables for update
    $pid = $_REQUEST['pid'];
    $pava = ($_REQUEST['pava'] - $_REQUEST['pquantity']);

    // Assigning User Values to Variables for insert
    ...
    if($conn->query($sqlin) == TRUE){
      ...
      $msg = '<div class="alert alert-danger col-sm-6 ml-5 mt-2" role="alert">Unable to sell product</div>';
    }
    
    // Update Asset data for available product after sell
